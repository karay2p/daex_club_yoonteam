...
import { Users, Mail, Target, CheckSquare, Eye } from 'lucide-react'
...
export default function MembersPage() {
  ...
  const getMemberStats = (memberId: string) => {
    ...
    const submissionRate = TOTAL_MISSIONS === 0 ? 0 : Math.round((memberSubs.length / TOTAL_MISSIONS) * 100)
    return { submissionCount: memberSubs.length, attendanceCount: memberAtt.length, submissionRate }
  }
      ...
      <Card className={`p-5 hover:shadow-md transition-all duration-200 hover:-translate-y-0.5 ${isMe ? 'border-brand-200 ring-1 ring-brand-200' : ''}`}>
        <div className="flex items-start gap-3 mb-4">
          <div className="relative">
            ...
            {isMe && (
              <div className="absolute -top-1 -right-1 w-4 h-4 bg-green-400 rounded-full border-2 border-white" title="나" />
            )}
          </div>
