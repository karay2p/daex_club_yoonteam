    ...
    <DashboardLayout title="스케줄">
      ...
      {loading ? <LoadingSpinner /> : (
        <div className="space-y-8">
          ...
          {past.length > 0 && (
            <div>
              <h3 className="text-sm font-bold text-gray-400 uppercase tracking-wider mb-4">지난 일정</h3>
              <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
                {past.map((sch, i) => (
                  <Card key={sch.id} className="p-5 opacity-60">
                    <h4 className="font-semibold text-gray-600 mb-1">{sch.title}</h4>
